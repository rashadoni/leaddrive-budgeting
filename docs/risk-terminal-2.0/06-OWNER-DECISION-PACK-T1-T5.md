...
6. forbid caller-supplied `basisAmount` for the five controls (or bound it to `≤ max(|left|, |right|)`), else the tolerance is caller-loosenable into C under an "A" label — **implemented 2026-07-19 in the strongest form (IMPLEMENTATION-STATUS §44): the field is removed from `StatementControlInput` entirely, so the basis is always `max(|left|, |right|)` by construction; a type-level regression test locks the removal**;
7. pre-register absolute floors for every source currency actually present in the pilots (at least AZN, USD, EUR, and likely TRY, RUB) in both floor maps — **implemented 2026-07-22 as a shadow-only policy fixture: `SHADOW_STATEMENT_POLICY` now registers AZN/USD/EUR/GBP/TRY/RUB in both floor maps and pins `approval: "provisional"` by test. Non-AZN fixed materiality floors remain zero placeholders so condition #5 (scope-aware materiality) is not silently invented.**;
