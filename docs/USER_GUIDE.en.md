...
**BudgetPro is a CFO terminal for a holding of 60+ companies.**
...
| 📋 | **Board Deck** | Board snapshot (print / PPTX / PDF) |
...
When clicking a HeatMap cell, transforms into **indicator detail:**
...
4. At bottom — EVENTS feed (recent LLM calls) and MARKET (USD/AZN, EUR/AZN, Brent)
...
| Card | What It Does |
|---|---|
| **Data Import** `Phase 7.M Tier 7` | Drag-drop any xlsx → AI determines type (P&L/BS/CF/KPI/Land/CAPEX/Descriptions/Forecast) and routes to correct adapter. One screen instead of 5 different forms. |
...
| **Source Registry** | Drift-watchdog: list of permitted xlsx sources for ingest. |
...
- `no_budget_lines: 64` — no P&L sources
- `no_foreign_currency_lines: 35`
- `rollup_no_children: 22`
- `parse: 2`
- `out_of_range: 1`

**At bottom:** list of specific indicators with problems + one-line remediation.

**What to check before demo:**
1. **AGRO_COMMODITY_VOL** → 75 cells, needs investigation
2. **FP_INVENTORY_TURNS** → 49 cells, needs `inventory` in BS
3. **FP_YIELD_LOSS** → 49 cells, needs `raw_input` in production KPI
4. **AGRO_DROUGHT_RISK** → 37 cells, needs `drought_index` per entity
...
4. **Mandatory Reconciliation** — every import reconciled with source
...
### Where These Flags Appear (4 Channels, End-to-End Tested)
...
Registries for other entities **intentionally not populated** — we do NOT generate risks ourselves, waiting for real KRIs from holding's Risk Officer. There should be no fabricated data here: the financial CFO makes decisions based on these indicators.
...
- **LEGAL_CASES_ACTIVE** — open court cases registry: 54 cases. AZSF — defendant in 26 (29 open). CPC — defendant in 7 (8 open). EDEN — only plaintiff (4 open). Full registry in `Company.settings.courtDisputes`.

### What to Check
- In HeatMap 3 new columns appeared (AUDIT_CLOSED_PCT / AUDIT_MAJOR_OPEN / LEGAL_CASES_ACTIVE)
- Click red cell AZSF/AUDIT_MAJOR_OPEN → Variance Explainer should cite open Major findings in narrative
- Board Deck → "Critical alerts" section now contains compliance/legal warnings

> **Money-at-risk per case (AZN):** removed from indicators 2026-05-27. Regex covered only 4 out of 54 cases (7%) — misleading floor estimate. Will be re-implemented when full registry of claim amounts arrives from Hüquq Şöbəsi.

---

## 9.2.5 FX Risk — What Portion of Revenue Is Vulnerable to Exchange Rates
...
| AZSF / MALT / EDEN / HORIZON / PROMALT | — | — | — | ⚪ Pending | Awaiting N. Nəcəfzadə file "Müştəri İcmalı" (CARRYOVER L1) |

Only CPC has real data (calculated from client's forward plan). For other 5 entities we do NOT fill split intentionally — `REVENUE_FX_EXPOSURE` shows `unknown` until verified per-customer FX breakdown arrives.
...
**Link to `FX_IMPORTED_INPUT`** (cost-side): two indicators together will give **NET FX position** when everyone has revenue split. If cost ≈ revenue in same currency → natural hedge.
...
Besides HHI (mathematically correct but poorly communicates to CFO), added **direct concentration indicators** that are immediately readable:
...
- **TOP3_CUSTOMER_SHARE** — "long-tail health" (MALT 80% means almost nothing after top-3 — can't replace if all 3 leave)
...
**What it does:** describes the day's risk cluster in one phrase + lists "top worst" by sector. Accounts for qualitative risk flags.
...
**Languages:** EN / RU / AZ (toggle in top right corner of panel).

**Cache mechanism:** sha256 of prompt text + dataset hash. Change prompt → old cache automatically invalidated.
...
Note — recommendation #3 cites `subsidy_dependency` risk flag from Risk Registry page.
...
**Purpose:** IFRS-compliant log of all significant changes. Stored for 365 days.
...
- [ ] Theme toggle (sun/moon) in top right corner works

### Risk Terminal (`/budgeting/terminal`)
- [ ] Panel 1: AZSEKER tree expands, see 7 sub-cos with composite-score (59%, 65%, 83%, 80%, 15%, ...)
...
- [ ] Click red cell EDEN row, CUSTOMER_HHI column → Panel 3 shows formula `counterparty_hhi_customer`
...
- [ ] In HeatMap columns `AUDIT_CLOSED_PCT`, `AUDIT_MAJOR_OPEN`, `LEGAL_CASES_ACTIVE` exist
...
- [ ] Click red cell AZSF/AUDIT_MAJOR_OPEN → Variance Explainer cites Major findings
