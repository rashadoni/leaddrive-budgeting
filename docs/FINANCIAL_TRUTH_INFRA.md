...
| E.4 | Terminal locked-period banner | ✅ Done | Session 9 closure — `src/features/terminal/components/TerminalLockedPeriodBanner.tsx` mounted in `terminal/page.tsx`. Hits `/api/budgeting/period-locks`, finds lock for `matrix.period`, renders amber strip with Lock icon + period + reason + lockedBy + lockedAt via i18n (EN/RU/AZ). 6 happy-dom test cases lock null-render branches + render path + amber/cursor-help affordances + fetch-fail graceful degradation. |
