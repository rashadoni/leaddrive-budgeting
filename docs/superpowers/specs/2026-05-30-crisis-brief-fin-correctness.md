...
- **REVENUE_DROP_30 is a wash.** A pure volume shock scales revenue AND variable cogs together → gross margin % unchanged (only EBITDA/net margin worsen via fixed opex). Economically defensible but undramatic. Options: model it as a price shock (revenue down, cogs flat → margin compresses hard) or pair with a cost element.
