...
- **Modest holding swing (≈3 pts).** A 30% cost shock flips multiple margin cells red, but the full holding composite only moves 61→58 because it averages ~30 indicators/company (ESG, carbon, news, etc. don't react to a financial shock). **MITIGATED 2026-05-30 (user-approved):** added a **financial-stress sub-composite** («Финансовое здоровье») — a revenue-weighted composite over JUST the P&L-driven indicators (`isFinancialIndicator` = formula reads revenue/cogs/gross_profit/ebitda/net_income/opex). It swings ~2× harder: INPUT_COST_30 health 62→56 (vs full 61→58), AZN_DEVAL_20 62→55 (vs 61→58), DROUGHT_2026 62→59 (vs 61→59). Both numbers are shown in the crisis panel — the contrast («общий риск −3, но финансовое здоровье −7») is the honest CFO story. The swing stays bounded because the composite is status-band-based (green=100/amber=50/red=0) and AzerSheker's baseline is already stressed (several entities red/negative EBITDA can't fall further) — this is the truth of the data, not inflated. The cell cascade + per-company drops + AI narrative carry the visual drama.
