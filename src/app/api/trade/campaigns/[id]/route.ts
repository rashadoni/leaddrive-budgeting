...
export async function PATCH(
  ...
  // Stage 3 RLS — pre-check + guarded write + scope replace in ONE
  // org-scoped tx.
  return withOrgScope(orgId, async (tx) => {
    ...
    if (!isCampaignEditable(campaign.status)) {
      return NextResponse.json(
        {
          ok: false,
          error: `Campaign in status "${campaign.status}" is not editable`,
        },
        { status: 409 },
      );
    }
    ...
    if (claimed.count === 0) {
      return NextResponse.json(
        {
          ok: false,
          error: "Campaign changed concurrently and is no longer editable",
        },
        { status: 409 },
      );
    }
    if (parsed.scopes) {
      await tx.tradeCampaignScope.deleteMany({
    ...
    const updated = await tx.tradeCampaign.findFirst({
...
export async function DELETE(
  ...
  // Stage 3 RLS — pre-checks + guarded soft-delete in one org-scoped tx.
  return withOrgScope(orgId, async (tx) => {
    const campaign = await tx.tradeCampaign.findFirst({
      where: { id, organizationId: orgId, deletedAt: null },
    ...
    if (!isCampaignEditable(campaign.status)) {
      return NextResponse.json(
        {
          ok: false,
          error: `Campaign in status "${campaign.status}" cannot be deleted`,
        },
        { status: 409 },
      );
    }
    ...
    const ledgerRefs = await tx.tradeSpendLedger.count({
      where: { organizationId: orgId, campaignId: id, voidedAt: null },
    });
    if (ledgerRefs > 0) {
      return NextResponse.json(
        {
          ok: false,
          error: `Campaign has ${ledgerRefs} spend entries — void them before deleting`,
        },
        { status: 409 },
      );
    }
    ...
    const deleted = await tx.tradeCampaign.updateMany({
      where: {
        id,
        organizationId: orgId,
        deletedAt: null,
    ...
    if (deleted.count === 0) {
      return NextResponse.json(
        {
          ok: false,
          error: "Campaign changed concurrently and can no longer be deleted",
        },
        { status: 409 },
      );
    }
