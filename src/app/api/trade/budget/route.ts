...
export async function POST(request: NextRequest) {
  ...
  // Stage 3 RLS — the whole derive (reads, upsert loop, rebase, pacing
  // recompute) runs in ONE org-scoped tx. 30s timeout: bounded DB work,
  // 12 months × (upsert + plan-daily regen + channel rebase + recompute).
  return withOrgScope(
    orgId,
    async (tx) => {
      ...
      const revenue = await tx.budgetLine.groupBy({
        by: ["monthIndex"],
        where: {
      ...
      const existing = await tx.tradeBudgetPool.findMany({
        where: { organizationId: orgId, year, grainKey: "org" },
      ...
      for (const m of touched) {
        await recomputeTradePacing(tx, orgId, year, m)
      }
