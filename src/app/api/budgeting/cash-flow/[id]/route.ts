...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const old = await prisma.cashFlowEntry.findFirst({ where: { id, organizationId: orgId, deletedAt: null } })
  if (!old) return NextResponse.json({ error: "Not found" }, { status: 404 })

  const lock = await findFirstActiveLockInPeriods(prisma, orgId, containingPeriodKeys(old.year, old.month))
  ...
  const updated = await prisma.cashFlowEntry.findFirst({ where: { id, organizationId: orgId } })
