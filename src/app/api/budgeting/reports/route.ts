...
export async function GET(req: NextRequest) {
  ...
  const [reports, total] = await Promise.all([
    prisma.savedBudgetReport.findMany({
      where: { organizationId: orgId },
      orderBy: { updatedAt: "desc" },
      skip: (page - 1) * limit,
      take: limit,
    }),
    prisma.savedBudgetReport.count({ where: { organizationId: orgId } }),
  ])

  return NextResponse.json({ success: true, data: reports, total, page, limit })
}
...
export async function POST(req: NextRequest) {
  ...
  let data
  try { data = createReportSchema.parse(body) } catch (e) {
  ...
  const report = await prisma.savedBudgetReport.create({
    data: {
