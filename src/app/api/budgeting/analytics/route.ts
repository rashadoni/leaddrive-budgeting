...
const COST_TYPE_SELECT = {
  key: true,
  label: true,
  costModelPattern: true,
  isShared: true,
  color: true,
} as const;
const BUDGET_DEPT_SELECT = {
  key: true,
  label: true,
  serviceKey: true,
  hasRevenue: true,
...
type BudgetLineWithRelations = Prisma.BudgetLineGetPayload<{
  select: typeof BUDGET_LINE_SELECT;
}>;
...
export async function GET(req: NextRequest) {
  ...
  return withOrgScope(orgId, async (tx) => {
    ...
    // Sequential inside the tx (an interactive tx serializes on one connection).
    ...
    const manualActuals = await tx.budgetActual.findMany({
      where:
        companyFilter.kind === "single"
          ? {
              planId,
              organizationId: orgId,
              companyId: { in: companyFilter.companyIds },
            }
    ...
    const hasAutoActual = lines.some(
      (l: BudgetLineWithRelations) => l.isAutoActual,
    );
    const hasAutoPlanned = lines.some(
      (l: BudgetLineWithRelations) => l.isAutoPlanned,
    );
    const costModel =
      hasAutoActual || hasAutoPlanned
        ? await loadAndCompute(orgId).catch(() => null)
        : null;
    ...
    if (plan.kind === "budget") {
      const actualsPlan = await tx.budgetPlan.findFirst({
        where: {
          organizationId: orgId,
          year: plan.year,
          kind: "actual",
      ...
      if (actualsPlan) {
        const aLines = await tx.budgetLine.findMany({
          where: {
        ...
        for (const l of aLines) {
          ...
          if (l.plannedAmount !== 0 && l.monthIndex != null)
            monthsWithData.add(l.monthIndex);
          const code = l.account?.code;
          if (code)
            actualByCode.set(
              code,
              (actualByCode.get(code) ?? 0) + l.plannedAmount,
            );
        }
    ...
    const { count: periodMonthCount, months: periodMonthNumbers } =
      getPeriodMonths(plan);
    const salesForecasts = hasAutoPlanned
      ? await tx.salesForecast.findMany({
          where: {
            organizationId: orgId,
            year: plan.year,
            month: { in: periodMonthNumbers },
          },
        })
      : [];
    const expenseForecasts = hasAutoPlanned
      ? await tx.expenseForecast.findMany({
          where: {
            organizationId: orgId,
            year: plan.year,
            month: { in: periodMonthNumbers },
    ...
    function getEffectivePlanned(line: BudgetLineWithRelations): number {
      return getEffectivePlannedPure(line, (l) => {
        const computed = computePlannedForLine(
          l,
          costModel,
          salesForecasts,
          periodMonthCount,
    ...
    if (hasAutoActual && costModel && plan) {
      ...
      const elapsedMonthIndices = computeElapsedMonthIndices(
        plan,
        curYear,
        curMonth,
      );
      ...
      for (const line of lines) {
        if (line.isAutoActual && line.costModelKey) {
          const monthlyAmount = resolveCostModelKey(
            costModel,
            line.costModelKey,
          );
          const amount = monthlyAmount * elapsedMonths;
          const key = `${line.account?.code ?? ""}||${line.lineType}`;
          autoActualByCategory.set(
            key,
            (autoActualByCategory.get(key) ?? 0) + amount,
          );
          // Phase 3.1 v1.2 — attribute one monthlyAmount per elapsed month.
          const monthly =
            autoActualMonthlyByCategory.get(key) ?? Array(12).fill(0);
    ...
    const forecastEntries = await tx.budgetForecastEntry.findMany({
    ...
    const expenseLines = lines
      .filter((l: { lineType: string }) => l.lineType === "expense")
      .filter(isLeaf);
    const revenueLines = lines
      .filter((l: { lineType: string }) => l.lineType === "revenue")
      .filter(isLeaf);
    const cogsLines = lines
      .filter((l: { lineType: string }) => l.lineType === "cogs")
      .filter(isLeaf);

    const totalExpensePlanned = expenseLines.reduce(
      (s: number, l: BudgetLineWithRelations) => s + getEffectivePlanned(l),
      0,
    );
    const totalRevenuePlanned = revenueLines.reduce(
      (s: number, l: BudgetLineWithRelations) => s + getEffectivePlanned(l),
      0,
    );
    // COGS totals — must be computed before totalPlanned
    const totalCOGSPlanned = cogsLines.reduce(
      (s: number, l: BudgetLineWithRelations) => s + getEffectivePlanned(l),
      0,
    );
    const totalPlanned = totalExpensePlanned; // OpEx only (COGS allocates same costs by service)
    const totalExpenseForecast = expenseLines.reduce(
      (s: number, l: BudgetLineWithRelations) =>
        s + (l.forecastAmount ?? getEffectivePlanned(l)),
      0,
    );
    const totalRevenueForecast = revenueLines.reduce(
      (s: number, l: BudgetLineWithRelations) =>
        s + (l.forecastAmount ?? getEffectivePlanned(l)),
      0,
    );
    // Forecast from ForecastEntries (monthly) — split by lineType
    const feRevenue = forecastEntries.filter(
      (e: { lineType: string }) => e.lineType === "revenue",
    );
    const feExpense = forecastEntries.filter(
      (e: { lineType: string }) => e.lineType === "expense",
    );
    const feCogs = forecastEntries.filter(
      (e: { lineType: string }) => e.lineType === "cogs",
    );
    // Use ForecastEntries when available, otherwise fall back to BudgetLine.forecastAmount
    const totalRevenueForecastFE =
      feRevenue.length > 0
        ? feRevenue.reduce(
            (s: number, e: { forecastAmount: number }) => s + e.forecastAmount,
            0,
          )
        : totalRevenueForecast;
    const totalExpenseForecastFE =
      feExpense.length > 0
        ? feExpense.reduce(
            (s: number, e: { forecastAmount: number }) => s + e.forecastAmount,
            0,
    ...
    const totalForecast =
      forecastEntries.length > 0
        ...
        : totalExpenseForecast;
    const manualActualTotal = manualActuals.reduce(
      (s: number, a: { actualAmount: number }) => s + a.actualAmount,
      0,
    );

    const totalCOGSForecast =
      feCogs.length > 0
        ? feCogs.reduce(
            (s: number, e: { forecastAmount: number }) => s + e.forecastAmount,
            0,
        ...
        : cogsLines.reduce(
            (
              s: number,
              l: { forecastAmount: number | null; plannedAmount: number },
            ) => s + (l.forecastAmount ?? l.plannedAmount),
            0,
          );
    ...
    const totalExpenseActual =
      autoActualExpense > 0
        ? autoActualExpense
        : manualExpenseActual > 0
          ? manualExpenseActual
          : planActualExpense;
    const totalRevenueActual =
      autoActualRevenue > 0
        ? autoActualRevenue
        : manualRevenueActual > 0
          ? manualRevenueActual
          : planActualRevenue;
    const totalCOGSActual =
      autoActualCOGS > 0
        ? autoActualCOGS
        : manualCOGSActual > 0
          ? manualCOGSActual
    ...
    const revAchievement =
      totalRevenuePlanned > 0
        ? Math.min((totalRevenueActual / totalRevenuePlanned) * 100, 150)
        : 100;
    const costDiscipline =
      totalActual > 0 && totalAllPlanned > 0
        ? Math.min((totalAllPlanned / totalActual) * 100, 150)
        : 100;
    const executionPct = Math.max(
      0,
      Math.round(revAchievement * 0.6 + costDiscipline * 0.4),
    );
    ...
    if (plan.periodType === "annual") {
      ...
      monthsElapsed = Math.max(
        1,
        Math.min(3, currentMonthNum - quarterStartMonth + 1),
      );
    }
    const yearEndProjection =
      totalActual > 0
        ? (totalActual / monthsElapsed) * periodMonths
        : totalForecast;
    // Margin year-end projection — project operating profit, not just expenses
    const marginYearEndProjection =
      monthsElapsed > 0
        ? (marginActual / monthsElapsed) * periodMonths
        : marginPlanned;
    ...
    for (const l of lines) {
      if (!l.parentId) {
        ...
        const children = lines.filter(
          (c: BudgetLineWithRelations) => c.parentId === l.id,
        );
        for (const c of children) {
          parentLookup.set(
            `${c.account?.code ?? ""}||${c.lineType}`,
            l.account?.code ?? "",
          );
        }
      }
    ...
    for (const l of lines) {
      ...
      const rawIdx =
        lAny.monthIndex ??
        (typeof lAny.sortOrder === "number" ? lAny.sortOrder % 100 : null);
      const monthIdx =
        rawIdx != null && rawIdx >= 0 && rawIdx < 12 ? rawIdx : 0;
    ...
    for (const [legacyKey, amount] of autoActualByCategory) {
      ...
      const totalPlanned = codeKeys.reduce(
        (s, k) => s + (categoryMap.get(k)?.planned ?? 0),
        0,
      );
      ...
      const monthlyArr =
        autoActualMonthlyByCategory.get(legacyKey) ?? Array(12).fill(0);
      if (totalPlanned > 0) {
        for (const k of codeKeys) {
          ...
          for (let i = 0; i < 12; i++)
            entry.monthlyActual[i] += monthlyArr[i] * share;
        }
    ...
    const byCategory = Array.from(categoryMap.entries()).map(([key, val]) => {
      ...
      const monthlyActual = isActualsPlan
        ? val.monthlyPlanned
        : val.monthlyActual;
      const variance =
        lineType === "revenue" ? actual - val.planned : val.planned - actual;
      ...
      return {
        ...
        monthlyPlanned: val.monthlyPlanned,
        monthlyActual,
        actualAvailable,
      };
    });
    ...
    const perCategoryActualsAvailable = byCategory.some(
      (c) => c.actualAvailable,
    );
    ...
    const expenseExecutionPct =
      totalExpensePlanned > 0
        ? (totalExpenseActual / totalExpensePlanned) * 100
        : 0;
    // Revenue execution % — how much of revenue target was achieved (<100% = shortfall)
    const revenueExecutionPct =
      totalRevenuePlanned > 0
        ? (totalRevenueActual / totalRevenuePlanned) * 100
        : 0;
    ...
    if (costTypes.length > 0) {
      ...
      const matrixLines = lines.filter(
        (l: BudgetLineWithRelations) =>
          l.costTypeId || (l.lineType === "revenue" && l.departmentId),
      );
      ...
      if (plan) {
        if (plan.periodType === "monthly") elapsedMonths2 = 1;
          ...
          if (
            curYear2 > plan.year ||
            (curYear2 === plan.year && curMonth2 > qEnd)
          )
            elapsedMonths2 = 3;
          else if (curYear2 === plan.year && curMonth2 >= qStart)
            elapsedMonths2 = curMonth2 - qStart + 1;
      ...
      for (const line of matrixLines) {
        ...
        const ctKey =
          line.lineType === "revenue" ? "_revenue" : ct?.key || "unknown";
        const ctLabel =
          line.lineType === "revenue"
            ? "Revenue"
            : ct?.label || (line.account?.name ?? line.account?.code ?? "");
        ...
        if (line.isAutoActual && costModel && ct?.costModelPattern) {
          const resolvedKey = dept?.serviceKey
            ...
            : ct.costModelPattern.includes("{dept}")
              ? null
              : ct.costModelPattern;
          if (resolvedKey) {
          ...
          const manualAmount = autoActualByCategory.has(catKey)
            ? 0
            : manualActuals
                .filter(
                  (a: BudgetActualRow) =>
                    a.category === lineAccountCode &&
                    a.lineType === line.lineType,
                )
                .reduce(
                  (s: number, a: BudgetActualRow) => s + a.actualAmount,
                  0,
                );
      ...
      for (const [, cell] of cellMap) {
        const variance =
          cell.lineType === "revenue"
            ? cell.actual - cell.planned
            : cell.planned - cell.actual;
        const variancePct =
          cell.planned > 0 ? (variance / cell.planned) * 100 : 0;
        ...
        const rt = matrixRowTotals[cell.costTypeKey] ?? {
          planned: 0,
          actual: 0,
          forecast: 0,
          variance: 0,
        ...
        if (cell.lineType !== "revenue") {
          ...
          const ct2 = matrixColTotals[colKey] ?? {
            planned: 0,
            actual: 0,
            forecast: 0,
            variance: 0,
    ...
    const matrix = {
      costTypes: costTypes.map((ct: BudgetCostTypeRow) => ({
        key: ct.key,
        label: ct.label,
        isShared: ct.isShared,
        color: ct.color,
      })),
      departments: departments.map((d: BudgetDepartmentRow) => ({
        key: d.key,
        label: d.label,
