...
const STATUS_PILL: Record<SectionStatus, { color: string; label: string }> = {
  complete: { color: "bg-emerald-500/10 text-emerald-600 border-emerald-500/30", label: "Complete" },
  partial: { color: "bg-amber-500/10 text-amber-600 border-amber-500/30", label: "Partial" },
  missing: { color: "bg-red-500/10 text-red-600 border-red-500/30", label: "Missing" },
  n_a: { color: "bg-gray-500/10 text-gray-500 border-gray-500/30", label: "N/A" },
};
...
export function OnboardingCompletenessDashboard() {
  ...
  if (companiesLoading || !companies) {
    return (
      <div className="p-8 flex items-center gap-2 text-muted-foreground">
        <Loader2 className="animate-spin h-4 w-4" /> Loading companies…
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      <header>
        <h1 className="text-2xl font-bold mb-1">Onboarding Completeness</h1>
        <p className="text-sm text-muted-foreground">
          Auto-derived from DB state. Reflects current data after every
          import + recheck. See{" "}
          <code className="text-xs bg-muted px-1.5 py-0.5 rounded">docs/ONBOARDING_DATA_REQUIREMENTS.md</code>{" "}
        ...
        <div className="border-t border-border/60 p-4 space-y-2">
          {isLoading && (
            <div className="text-xs text-muted-foreground flex items-center gap-1">
              <Loader2 className="animate-spin h-3 w-3" /> Computing…
            </div>
          )}
          {data && (
            <>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    disabled={isFetching}
                    title="Re-derive completeness from current DB state"
...
function SectionRow({ section }: { section: SectionResult }) {
  const pill = STATUS_PILL[section.status];
  return (
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {section.rowCount > 0 && (
            <span className="text-[10px] text-muted-foreground tabular-nums">
              {section.rowCount.toLocaleString()} rows
            </span>
          )}
        </div>
