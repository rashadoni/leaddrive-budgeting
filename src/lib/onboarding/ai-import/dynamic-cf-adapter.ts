...
function resolveColumnsPartial(
  ...
  for (const c of columns) {
    if (c.role === "code") {
      ...
      // Phase 11.10 (2026-07-29) — honour the year the role declares. See the
      // long note in dynamic-bs-adapter.ts: stripping the year and taking the
      // FIRST match wrote a two-year sheet's wrong range under the requested
      // year and discarded the right one. Year-less roles stay eligible.
  ...
  if (monthCols.size === 0) {
    ...
    return { ok: false, reason: "No month columns in proposal" }
  }
...
export async function runDynamicCfAdapter(
  ...
  try {
    ...
    return {
      ...
      blocked: { reason: `dynamic CF detection failed (LLM error): ${msg}` },
      applyToDb: async () => ({ rowsInserted: 0 }),
    }
  }
  ...
  if (proposal.overallConfidence < 0.5) {
    return {
      ...
      // committed green with no data.
      blocked: {
        reason: `dynamic CF detection confidence ${proposal.overallConfidence.toFixed(2)} < 0.50 — refusing to guess the layout`,
      },
      applyToDb: async () => ({ rowsInserted: 0 }),
    }
  }
  ...
  if (!colsResult.ok) {
    ...
    if (colsResult.reason.startsWith("YEAR_MISMATCH:")) {
      ...
      return {
        summary: `Dynamic CF: sheet "${input.sheetName}" carries ${otherYears} columns, not ${input.year} — skipped`,
        itemCount: 0,
        warnings: [
          `Dynamic CF: every month column on sheet "${input.sheetName}" belongs to ${otherYears}, but the import year is ${input.year} — skipped. Re-run the import with year=${otherYears.split(",")[0]} to load it.`,
        ],
        applyToDb: async () => ({ rowsInserted: 0 }),
      }
    }
    return {
      ...
      blocked: {
        reason: `dynamic CF column mapping incomplete: ${colsResult.reason}`,
      },
      applyToDb: async () => ({ rowsInserted: 0 }),
    }
  }
