...
describe("metric-validation-rules — catalog shape", () => {
  it("ships 21 operational metric rules (the 13 indicators × 1-2 inputs each)", () => {
    ...
    // teachers = 2; some single-metric like AGRO_DROUGHT_RISK). Total
    // metrics that must be enterable through the data-entry UI: 21.
    expect(OPERATIONAL_METRIC_RULES).toHaveLength(21)
