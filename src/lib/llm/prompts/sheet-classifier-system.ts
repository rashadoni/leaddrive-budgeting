...
1. **dataType** — exactly one of:
   ...
   - "ASSUMPTIONS"    = Budget-assumptions / drivers sheet — a label/value table of the PARAMETERS a budget is built on, not results of it: FX rates, inflation, tax and interest rates, tariffs, wage or price growth, yields, capacity utilisation, imported-input share. Columns are typically parameter/name + value, optionally unit, period, category, company, notes/source. Tab names like "Assumptions", "Drivers", "Fərziyyələr", "Допущения", "Предпосылки", "Параметры", "Inputs". CRITICAL distinctions: an assumption has NO date column and one value per parameter — if each row is a dated measurement, that is OPS_FACTS; if rows are P&L line items across month columns, that is PLF; if rows are realized crop or production output, that is KPI_FARMING / KPI_PROCESSING. A rate or a norm stated once, to be applied forward, is ASSUMPTIONS.
  ...
  "classifications": [
    {
      ...
      "dataType": "PLF|BS|CF|KPI_FARMING|KPI_PROCESSING|CAPEX|SALES|LAND_REGISTRY|DESCRIPTIONS|INFO_SUMMARY|COMPANIES|OPS_FACTS|BUDGET_ACTUALS|SALES_FORECAST|COUNTERPARTY|LEGAL_CASES|AUDIT_FINDINGS|RISK_REGISTER|ASSUMPTIONS|UNKNOWN",
