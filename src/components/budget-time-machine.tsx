        ...
        <CardContent className="py-3 px-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>
                {t.rich("timeMachineSummary", {
                  count: timePoints.length,
                  strong: (chunks) => <strong>{chunks}</strong>,
                })}
              </span>
              <span className="text-xs opacity-60">
                {t("timeMachineLast", { when: formatTimePoint(lastPoint.timestamp) })}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col items-center">
                <span className="text-sm font-medium text-purple-700 dark:text-purple-300">
                  {currentPoint ? formatTimePoint(currentPoint.timestamp) : t("timeMachineSelectPoint")}
                </span>
                {currentPoint && (
                  <span className="text-xs text-muted-foreground">
                    {currentPoint.summary} · {t("timeMachineCheckpoint", { index: currentIndex + 1, total: timePoints.length })}
                  </span>
                )}
              </div>
