...
- **NEVER claim a screenshot shows something without concrete evidence.** When verifying a visual change (Chrome MCP, computer-use, screenshots), do not assert «вот, X виден» unless you can point to the exact pixel/glyph/coordinate and describe what makes it identifiable. If unsure: zoom tighter, cross-check the accessibility tree via `find` (titles/aria-labels survive), or say «не вижу — проверю иначе». Past failure 2026-05-27: Claude saw a 1-letter `m` provenance marker, called it `⏳`, user trusted the false positive. The fix when a marker reads ambiguously is to **make the rendering more visible** (filled dot + ring beats a 9px emoji), not to claim it «уже видно». See `.claude/memory/feedback_no_screenshot_lies.md`.
